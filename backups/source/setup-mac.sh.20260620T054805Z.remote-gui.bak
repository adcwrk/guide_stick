...
REQUIRED_LABEL="${REQUIRED_USB_LABEL:-THKAILAR}"
REQUIRED_UUID="${REQUIRED_USB_UUID:-6676-08D4}"
REQUIRED_MOUNT="${REQUIRED_USB_MOUNT:-/mnt/usb}"
PULL_OPTIONAL="${PULL_OPTIONAL_MODELS:-0}"

mkdir -p "$LOG_DIR" "$REPORT_DIR" "$CONFIG_DIR" "$DOCUMENTS_DIR" "$BACKUP_DIR" "$INSTALLER_DIR" "$OLLAMA_MODELS_DIR" "$ANYTHINGLLM_DATA_DIR/storage"
